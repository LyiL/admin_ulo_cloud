...
@Component({
  ...
  providers: [ AgencyDetailDBService, LoadTableDbService,LoadBankLinkNo]
})
export class AgencyAccountComponent implements OnInit{
  ...
  ngOnInit(){
    let chanParams:any = {chanCode:this.ctr.params['chanCode']};
    //初始化代理商户帐户信息
   ...
   */
  onBack(){
    let _params = { step:0,isEdit:true,id:this.ctr.params['id'],orgId:this.ctr.params['orgId'],name:this.ctr.params['name'],chanCode:this.ctr.params['chanCode'],parentChanCode:this.ctr.params['parentChanCode']}
  ...
  onSaveAccounts(){
    if(this.hasData()){
      ...
      this.accountDB.data.forEach(_data=>{
        ...
        _data['bankCode'] = this.ctr.params['bankCode'];
      ...
      this.agencyDetailDBService.saveAccountInfos(_commitData).subscribe(res=>{
        if(res && res['status'] == 200){
          this.snackBar.alert('保存帐户信息成功！');
          let _params = {step:2,isEdit:true,source:this.ctr.params['source'],orgId:this.ctr.params['orgId'],chanCode:this.ctr.params['chanCode'],bankCode:this.ctr.params['bankCode'],bankName:this.ctr.params['bankName'],parentChanCode:this.ctr.params['parentChanCode']};
          this.sidenavService.resetPageParams(_params);
          this.ctr.params = _params;
          this.ctr.onStep(2);
        }else{
