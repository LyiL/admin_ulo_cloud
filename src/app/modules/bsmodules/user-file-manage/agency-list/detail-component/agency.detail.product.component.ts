...
import {AgencyDetailProductFormComponent} from "./agency.detail.product.form.component";
...
@Component({
  selector: 'agency-detail-product',
  templateUrl: 'agency.detail.product.component.html',
  providers: [LoadTableDbService, AgencyDetailDBService, CommonDBService]
})
export class AgencyDetailProductComponent implements OnInit {
  public ctr: ULODetailContainer;
  @ViewChild('agencyProdTable') agencyProdTable: MdTableExtend;
  public productData: any;

  public agencyProdColumns: Array<any> = [{
    title: '产品名称',
    name: 'combName'
  }, {
    title: '产品代码',
  ...
  public agencyProdActions: any = {
    actions: [{
      ...
      hide: ((row: any, target: any) => {
        if (!this.helper.btnRole('AGENTPRODCFGCOMMIT')) {
          return true;
        }
        //非优络的不显示此按钮
        if(!this.helper.hasConfigValueMatch('CLOUD_ULO_BANK_NO',this.uloCode)){
          return true;
        }
      ...
      click: (function (row: any) {
        const detailTable = this.agencyProdTable.detailTables[row['table_id']];
        if (detailTable) {
          let flag: boolean = false;
          let tableData = this.agencyProdTable.detailTables[row['table_id']].database.data;
          ...
          if (data) {//判断是否修改了数据，没有就不需要提交到后端
            this.agencyDetailDBService.saveAgencyProdCfgs({id: row['id'], tradetypes: data}).subscribe(res => {
      ...
      btnDisplay:'新增',
      hide:((row:any,target:any)=>{
        if(!this.helper.btnRole('AGENTPRODPAYTYPE')){
          return true;
        }
        //非优络的不显示此按钮
        if(!this.helper.hasConfigValueMatch('CLOUD_ULO_BANK_NO',this.uloCode)){
      ...
      hide: (function (row: any, target: any) {
        if (!this.helper.btnRole('AGENTCLOSEPROD')) {
          return true;
        }
        //为优络时才进入这个判断
        if(this.helper.hasConfigValueMatch('CLOUD_ULO_BANK_NO',this.uloCode)){
      ...
      click: (function (row: any) {
        ...
        if (row['state'] == 1 || row['state'] == 4) {
          this.agencyDetailDBService.saveAgencyProdClose({id: row['id']}).subscribe(res => {
            if (res && res['status'] == 200) {
      ...
      hide: (function (row: any) {
        if (!this.helper.btnRole('AGENTOPENPROD')) {
          return true;
        }
      ...
      click: (function (row: any) {
        ...
        if (row['state'] == 3) {
          this.agencyDetailDBService.saveAgencyProdOpen({
            userNo: row['userNo'],
            combId: row['id'],
            tradetypes: detailTable && detailTable.database.data
          }).subscribe(res => {
            if (res && res['status'] == 200) {
      ...
      btnDisplay: '通过',
      hide: (function (row: any, target: any) {
        if (!this.helper.btnRole('AGENTPRODEXAMINE')) {
          return true;
        }
        //为优络时才进入这个判断
        if(this.helper.hasConfigValueMatch('CLOUD_ULO_BANK_NO',this.uloCode)){
      ...
      click: (function (row: any) {
        ...
        if (row['state'] == 0) {
          this.agencyDetailDBService.saveAgencyProdExamine({id: row['id'], state: 1}).subscribe(res => {
            if (res && res['status'] == 200) {
      ...
      btnDisplay: '拒绝',
      hide: (function (row: any, target: any) {
        if (!this.helper.btnRole('AGENTPRODREFUSE')) {
          return true;
        }
        //为优络时才进入这个判断
        if(this.helper.hasConfigValueMatch('CLOUD_ULO_BANK_NO',this.uloCode)){
      ...
      click: (function (row: any) {
        ...
        if (row['state'] == 0) {
          this.agencyDetailDBService.saveAgencyProdExamine({id: row['id'], state: 4}).subscribe(res => {
            if (res && res['status'] == 200) {
      ...
      btnDisplay: '重新开通',
      hide: (function (row: any, target: any) {
        if (!this.helper.btnRole('AGENTRESETPROD')) {
          return true;
        }
        //为优络时才进入这个判断
        if(this.helper.hasConfigValueMatch('CLOUD_ULO_BANK_NO',this.uloCode)){
      ...
      click: (function (row: any) {
        ...
        if (row['state'] == 2 || row['state'] == 4) {
          this.agencyDetailDBService.saveAgencyProdResetOpen({id: row['id']}).subscribe(res => {
            if (res && res['status'] == 200) {
  ...
  //产品明细配置
  public agencyProdDetailCfg: MdTableExtendConfig = {
    columns: [{
      name: 'transId',
      title: '支付类型',
      type: 'select',
  ...
  constructor(public agencyProdDB: LoadTableDbService,
  ...
  public uloCode:any;
  ngOnInit(): void {
    this.uloCode = this.ctr && this.ctr.params['bankCode'];
  ...
  onRowClick(e) {
    if (e && e.parentData && e.detailCtr) {
      ...
      this.agencyDetailDBService.loadAgencyProdPayTypeData({id: row['id'], state: row['state']}).subscribe(res => {
        if (res && res['status'] == 200) {
  ...
  public reload(params) {
    if (params) {
      this.agencyDetailDBService.loadAgencyProdData({userNo: params['chanCode']}).subscribe(res => {
        if (res && res['status'] == 200 && res['data'] && res['data'].length > 0) {
          this.productData = res['data'];
          this.agencyProdDB.dataChange.next(res['data']);
        }
      });
    }
  }
   ...
   * 添加产品配置
   */
  public onAddAgencyProdCfg() {
    let params = this.sidenavService.getPageParams();
    if (!params) {
      return;
    }
    let _prodFormWin = this.pupop.openWin(AgencyDetailProductFormComponent, {
      title: '添加产品',
      width: '1400px',
      height: '0',
   ...
   */
  onBack(){
    let _params = {step:1,isEdit:true,source:this.ctr.params['source'],orgId:this.ctr.params['orgId'],chanCode:this.ctr.params['chanCode'],name:this.ctr.params['name'],bankCode: this.ctr.params['bankCode'],bankName:this.ctr.params['bankName'],parentChanCode:this.ctr.params['parentChanCode'],id:this.ctr.params['id']};
  ...
  onToNext(){
    ...
    let _params = {step:3,isEdit:true,source:this.ctr.params['source'],orgId:this.ctr.params['orgId'],chanCode:this.ctr.params['chanCode'],bankCode:this.ctr.params['bankCode'],bankName:this.ctr.params['bankName'],parentChanCode:this.ctr.params['parentChanCode']};
    this.sidenavService.resetPageParams(_params);
    this.ctr.params = _params;
    this.ctr.onStep(3);
  }
