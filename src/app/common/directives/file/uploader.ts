...
export class MdUploader {
  ...
  _progress:EventEmitter<any> = new EventEmitter();
  _abort:EventEmitter<any> = new EventEmitter();
  _error:EventEmitter<any> = new EventEmitter();
  ...
  setOptions(options: any): void {
    ...
    this.fileSuffix = options.fileSuffix != null ? options.fileSuffix : '*';
    if (!this.multiple) {
      this.maxUploads = 1;
    }
  }
  ...
  addFilesToQueue(files: File[]): void {
    files.forEach((file: File, i: number) => {
      if(!this.hasFileSuffix(file.name)){
        this._error.emit({error:1000,message:'上传错误, 请以正确格式上传！('+this.fileSuffix+')'});
        return;
      }
      if(file.size > this.transBytes()){
        this._error.emit({error:1001,message:'您上传的文件过大,请上传【'+this.fileSize+'】之内的文件'});
        return;
      }
  ...
  transBytes():number{
    let _size:number = Number(this.fileSize.replace(/^(\d+)(K|M|G|TB)+/,'$1'));
    let _unit = this.fileSize.replace(/^(\d+)(K|KB|M|G|TB)+/,'$2').toLocaleUpperCase();
    let _defByte = 1024;
    if(_unit == 'K' || _unit == 'KB'){
   ...
   * @returns {boolean}
   */
  hasFileSuffix(fileName){
    if(this.fileSuffix == '*'){
      return true;
    }
    let _suffix = fileName.substring(fileName.lastIndexOf('.'));
    let _fileSuffixs:Array<any> = this.fileSuffix.split(';');
    let flag = false;
